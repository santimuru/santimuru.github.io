const translations = {
  en: {
    ...
    'proj4.desc': 'End-to-end churn analysis combining <strong>Kaplan-Meier + Cox Proportional Hazards</strong> survival models with an <strong>XGBoost classifier (AUC 0.842)</strong>. Features 6 interactive sections: survival curves by segment, hazard ratio forest plot, SHAP beeswarm, individual customer waterfall explanations, a churn simulator, and a revenue impact calculator. Applied similar techniques in production at Montecable, achieving a <strong>13% churn reduction</strong>.',
  ...
  es: {
    ...
    'proj4.desc': 'Análisis de churn end-to-end que combina modelos de supervivencia <strong>Kaplan-Meier + Cox Proportional Hazards</strong> con un <strong>clasificador XGBoost (AUC 0.842)</strong>. Incluye 6 secciones interactivas: curvas de supervivencia por segmento, forest plot de hazard ratios, SHAP beeswarm, explicaciones waterfall por cliente, simulador de churn y calculadora de impacto de ingresos. Técnicas similares aplicadas en producción en Montecable, logrando una <strong>reducción del 13% en el churn</strong>.',

    'proj5.title': 'Inteligencia de Reseñas de Clientes',
    'proj5.context': 'Open Source · Yelp Review Full Dataset · HuggingFace',
    'proj5.demo': 'Demo en vivo',
    'proj5.desc': 'Pipeline NLP que detecta <strong>riesgo de churn en texto de reseñas</strong> mediante 4 métodos progresivos — TF-IDF + LR (<strong>AUC 0.981</strong>), Sentence Embeddings + XGBoost (0.945), Zero-Shot LLM (0.930) y Features Ingenierizados (0.925). Incluye explicaciones SHAP sobre texto, un analizador de texto en tiempo real y <strong>Google Gemini API</strong> para categorización estructurada de quejas.',

    'proj6.title': 'Predicción de Valor de Vida del Cliente',
    'proj6.context': 'Open Source · UCI Online Retail Dataset',
    'proj6.demo': 'Demo en vivo',
    'proj6.desc': 'Pipeline end-to-end de predicción de CLV usando modelos probabilísticos <strong>BG/NBD + Gamma-Gamma</strong> sobre el dataset UCI Online Retail. El dashboard incluye segmentación RFM, mapa de calor de retención por cohortes, proyecciones de CLV a 6 y 12 meses, y un simulador de clientes. Proyecta <strong>£8M+ en CLV a 12 meses</strong> para 2.790 clientes — el 20% superior representa <strong>más del 65% del CLV total</strong>.',
  ...
  ja: {
    ...
    'proj4.demo': 'ライブデモ',
    'proj4.desc': '<strong>Kaplan-Meier + Cox比例ハザード</strong>生存モデルと<strong>XGBoost分類器（AUC 0.842）</strong>を組み合わせたend-to-endの解約分析。セグメント別生存曲線・ハザード比フォレストプロット・SHAP beeswarm・顧客個別のウォーターフォール説明・解約シミュレーター・収益インパクト計算機の6つのインタラクティブセクションを搭載。同様の手法をMontecableの本番環境に適用し、<strong>解約率13%削減</strong>を実現。',

    'proj5.title': '顧客レビュー・インテリジェンス',
    'proj5.context': 'オープンソース · Yelp Review Full データセット · HuggingFace',
    'proj5.demo': 'ライブデモ',
    'proj5.desc': 'レビューテキストから<strong>顧客の解約リスクを検知</strong>するNLPパイプライン。TF-IDF + LR（<strong>AUC 0.981</strong>）・センテンス埋め込み + XGBoost（0.945）・ゼロショットLLM（0.930）・特徴量エンジニアリング（0.925）の4つの手法を段階的に比較。SHAPによるテキスト説明・リアルタイムテキスト分析・<strong>Google Gemini API</strong>による構造化クレーム分類を搭載。',

    'proj6.title': '顧客生涯価値予測',
    'proj6.context': 'オープンソース · UCI Online Retail データセット',
    'proj6.demo': 'ライブデモ',
    'proj6.desc': 'UCI Online RetailデータセットにおいてBG/NBD + Gamma-Gamma確率モデルを用いたCLV予測パイプライン。RFMセグメンテーション・コホート別リテンションヒートマップ・6/12ヶ月CLV予測・顧客シミュレーターを搭載したダッシュボード。2,790顧客の<strong>12ヶ月CLV合計£8M超</strong>を予測 — 上位20%の顧客が<strong>CLV全体の65%以上</strong>を占める。',

    'proj7.title': '東京不動産マーケット エクスプローラー',
    'proj7.context': 'オープンソース · 国土交通省公開統計に基づくモデル',
    'proj7.demo': 'ライブデモ',
    'proj7.desc': '東京<strong>23特別区</strong>の不動産市場（2020〜2024年）をインタラクティブに可視化するダッシュボード。<strong>Pydeckによる3Dマップ</strong>で区別価格を表示し、Plotlyグラフでトレンド・分布を分析、区単位のディープダイブ、<strong>k近傍法による価格推定</strong>（P10/P50/P90）機能を搭載。国土交通省の公開統計に基づきモデル化した<strong>約5万件の取引データ</strong>を使用 — <strong>国土交通省 不動産情報ライブラリAPI</strong>への接続に対応した設計。',
